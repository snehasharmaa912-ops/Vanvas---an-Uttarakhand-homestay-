  ...
  useEffect(() => {
    ...
    fetch(API_URL)
      ...
      .catch(err => {
        console.error(err)
        setLoading(false)
        setToast({ visible: true, message: 'Could not load homestays. Please try again.', type: 'error' })
      })
  }, [])

  // Live updates: when any host adds, edits, or removes a stay, reflect it
  // here immediately without requiring a page refresh.
  useEffect(() => {
    socket.connect()

    const onCreated = stay => setStays(prev => [stay, ...prev])
    const onUpdated = stay => setStays(prev => prev.map(s => (s._id === stay._id ? stay : s)))
    const onDeleted = ({ id }) => setStays(prev => prev.filter(s => s._id !== id))
