      ...
      <div className="section-pad max-w-4xl">
        ...
        <div id="about" className="mb-14">
          ...
          <p className="text-[#555] text-lg leading-relaxed max-w-2xl">
            ...
            that travelers never find because the hosts have no digital presence. We are changing that.
        ...
        <div id="how-it-works" className="mb-14 pt-4">
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {STEPS.map(function (step) {
              return (
                <div key={step.number} className="flex gap-4 bg-white dark:bg-[#1a4a31] border border-[#e8dfc8] dark:border-[#2d7a4f]/30 rounded-2xl p-5">
                  <span className="text-2xl font-bold text-[#2d7a4f]/30 display-font">{step.number}</span>
                  <div>
                    <h3 className="font-semibold text-[#1c1c1c] dark:text-white text-sm mb-1">{step.title}</h3>
                    <p className="text-xs text-[#777] dark:text-white/60 leading-relaxed">{step.desc}</p>
                  </div>
                </div>
              )
            })}
          ...
          <div className="bg-white dark:bg-[#1a4a31] border border-[#e8dfc8] dark:border-[#2d7a4f]/30 rounded-2xl p-6 flex flex-col sm:flex-row gap-6 justify-between">
            <div>
              <p className="text-sm text-[#666] dark:text-white/70 leading-relaxed mb-3">
                Have a question, partnership idea, or want to list your homestay? Reach out, I would love to hear from you.
              </p>
              <p className="text-sm text-[#1c1c1c] dark:text-white font-medium">Email: sharmasnehaa08@gmail.com</p>
              <p className="text-sm text-[#1c1c1c] dark:text-white font-medium">Location: Dehradun, Uttarakhand</p>
            </div>
            <a href="mailto:sharmasnehaa08@gmail.com" className="btn-primary self-start sm:self-center whitespace-nowrap">
              Email us
            </a>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-4">
            {TEAM.map(function (member) {
              return (
                <div key={member.name} className="flex items-center gap-3 bg-white dark:bg-[#1a4a31] border border-[#e8dfc8] dark:border-[#2d7a4f]/30 rounded-2xl px-5 py-4">
                  <div className={'w-10 h-10 rounded-full ' + member.color + ' flex items-center justify-center font-bold text-sm flex-shrink-0'}>
                    {member.initials}
                  </div>
                  <div>
                    <p className="font-semibold text-[#1c1c1c] text-sm">{member.name}</p>
                    <p className="text-xs text-[#888]">{member.role}</p>
                    {member.instagram && member.linkedin && (
                      <div className="flex gap-3 mt-1.5">
                        <a href={member.instagram} target="_blank" rel="noreferrer" className="text-xs text-[#a96f2b] hover:underline">Instagram</a>
                        <a href={member.linkedin} target="_blank" rel="noreferrer" className="text-xs text-[#2d7a4f] hover:underline">LinkedIn</a>
                      </div>
                    )}
                  </div>
                </div>
