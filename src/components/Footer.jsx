...
const socials = [
  ...
  { label: 'Twitter',    href: '#', icon: socialTwitter },
...
function FooterColumn(props) {
  return (
    <div>
      <h4 className="text-xs font-semibold text-white/40 uppercase tracking-widest mb-4">{props.heading}</h4>
      <ul className="space-y-2.5">
        {props.links.map(function (link) {
          return (
            <li key={link.label}>
              <Link to={link.to} className="text-sm text-white/60 hover:text-white transition-colors duration-150">
                {link.label}
              </Link>
            </li>
          )
